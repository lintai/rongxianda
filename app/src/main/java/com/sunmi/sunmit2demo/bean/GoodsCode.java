...
public class GoodsCode {
    ...
    public final static int MODE_5 = 5; //用户自行添加商品
...
//            2.1f, 5.1f, 4.1f, 3.1f, 6.1f, 0.02f,
    ...
    private void addDefaultGoods() {
        ...
        for (int i = 0; i < species.length; i++) {
            ...
            for (int i1 = 0; i1 < species[i]; i1++) {
                ...
                if (i == MODE_3) {
                    logo = dialog_logo[i1 + species[MODE_2]];
                }
                if (string[i1 + j] instanceof Integer) {
                    add(code[i1 + j], icon[i1 + j], ResourcesUtils.getString(MyApplication.getInstance(), (Integer) string[i1 + j]), price[i1 + j], i, logo);
                } else {
                    add(code[i1 + j], icon[i1 + j], (String) string[i1 + j], price[i1 + j], i, logo);
                }
            }
        }
    }
    ...
    public void add(String code, int imageId, String resString, float price, int mode, int logo) {
        GvBeans gvBeans = new GvBeans(imageId, resString, ResourcesUtils.getString(R.string.units_money) + price, code, 100, "", mode);
        if (logo != 0) {
